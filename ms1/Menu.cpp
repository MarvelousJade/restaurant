...
using namespace std;
namespace seneca {
Menu::Menu(const char *title, const char *exitOption, size_t indentationsCount,
           size_t indentationSize)
    : m_indentationsCount(indentationsCount),
      m_indentationSize(indentationSize), m_menuItemsCount(0),
...
Menu &Menu::operator<<(const char *content) {
  if (m_menuItemsCount < MaximumNumberOfMenuItems) {
    m_menuItems[m_menuItemsCount] = new MenuItem(
        content, m_indentationsCount, m_indentationSize, m_menuItemsCount + 1);
    m_menuItemsCount++;
  }
  return *this;
}
